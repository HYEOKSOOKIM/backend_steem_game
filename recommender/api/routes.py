...
if APIRouter is not None:
    ...
    @recommend_router.get("/game/{app_id}/reviews")
    def recommend_game_reviews(
        app_id: int,
        ...
        pos_limit = max(1, min(100, int(positive_limit)))
        neg_limit = max(1, min(100, int(negative_limit)))

        try:
            from recommender.src.db import get_connection
            from recommender.src.localize import translate_en_to_ko_many
            ...
            with get_connection(_resolve_db_path(), readonly=True) as conn:
                ...
                def _fetch(voted_up: int, limit: int) -> list[dict[str, Any]]:
                    rows = conn.execute(
                        """
                        SELECT review_id, cleaned_text, review_text, voted_up, votes_up, playtime_forever, review_date
                        FROM reviews
                        WHERE app_id = ? AND voted_up = ?
                        ORDER BY votes_up DESC, review_date DESC
                        LIMIT ?
                        """,
                        (app_id, voted_up, limit),
                    ).fetchall()
                    originals = [
                        str((r["cleaned_text"] or r["review_text"] or "")).strip()
                        for r in rows
                    ]
                    translated = translate_en_to_ko_many(originals)
                    out: list[dict[str, Any]] = []
                    for r, original, ko in zip(rows, originals, translated):
                        out.append(
                            {
                                "review_id": str(r["review_id"] or ""),
                                "voted_up": bool(r["voted_up"]),
                                "votes_up": int(r["votes_up"] or 0),
                                "playtime_forever": int(r["playtime_forever"] or 0),
                                "review_date": str(r["review_date"] or ""),
                                "text": original,
                                "text_ko": str(ko or original),
                            }
                        )
                    return out

                positive = _fetch(1, pos_limit)
                negative = _fetch(0, neg_limit)

            return JSONResponse(
                {
                    "app_id": int(game_row["app_id"]),
                    "display_name": str(game_row["display_name"] or app_id),

...
def _page(initial_query: str, initial_top_k: int) -> bytes:
    ...
    doc = (
        ...
        "      margin-bottom: 10px;\n"
        "    }\n"
        "    .review-tabs { display: flex; gap: 8px; margin: 8px 0 10px; }\n"
        "    .review-item { border: 1px solid #e5e7eb; border-radius: 10px; padding: 10px; margin-bottom: 8px; }\n"
        "    .review-meta { font-size: 12px; color: #64748b; margin-top: 6px; }\n"
        "    .mini-btn {\n"
        "      border: 1px solid #99f6e4;\n"
        "      background: #ecfeff;\n"
        "      color: #115e59;\n"
        ...
        "      const [reviewModal, setReviewModal] = React.useState({ open: false, loading: false, error: '', appId: null, gameName: '', tab: 'positive', positive: [], negative: [] });\n"
        "\n"
        "      React.useEffect(() => {\n"
        ...
        "        const appId = item && item.app_id;\n"
        "        if (!appId) return;\n"
        "        setReviewModal({ open: true, loading: true, error: '', appId: appId, gameName: item.display_name || item.name || '', tab: 'positive', positive: [], negative: [] });\n"
        "        try {\n"
        "          const res = await fetch('/api/recommend/game/' + appId + '/reviews?positive_limit=20&negative_limit=20');\n"
        "          const payload = await res.json();\n"
        ...
        "            loading: false,\n"
        "            error: '',\n"
        "            positive: payload.positive_reviews || [],\n"
        "            negative: payload.negative_reviews || []\n"
        "          }));\n"
        "        } catch (err) {\n"
        "          setReviewModal((prev) => ({ ...prev, loading: false, error: String(err) }));\n"
        "        }\n"
        "      }\n"
        "\n"
        "      function closeReviewModal() {\n"
        "        setReviewModal({ open: false, loading: false, error: '', appId: null, gameName: '', tab: 'positive', positive: [], negative: [] });\n"
        "      }\n"
        "\n"
        ...
        "            h('button', { className: 'mini-btn', onClick: () => openReviewModal(item) }, '리뷰 보기')\n"
        "          ),\n"
        ...
        "\n"
        "        reviewModal.open\n"
        "          ? h('div', { className: 'modal-backdrop', onClick: closeReviewModal },\n"
        "              h('div', { className: 'modal-card', onClick: (e) => e.stopPropagation() },\n"
        "                h('div', { className: 'modal-head' },\n"
        "                  h('strong', null, (reviewModal.gameName || '게임') + ' 리뷰'),\n"
        "                  h('button', { onClick: closeReviewModal }, '닫기')\n"
        "                ),\n"
        "                h('div', { className: 'review-tabs' },\n"
        "                  h('button', { className: 'tab-btn ' + (reviewModal.tab === 'positive' ? 'active' : ''), onClick: () => setReviewModal((prev) => ({ ...prev, tab: 'positive' })) }, '긍정 리뷰'),\n"
        "                  h('button', { className: 'tab-btn ' + (reviewModal.tab === 'negative' ? 'active' : ''), onClick: () => setReviewModal((prev) => ({ ...prev, tab: 'negative' })) }, '부정 리뷰')\n"
        "                ),\n"
        "                reviewModal.loading\n"
        "                  ? h('div', { className: 'meta' }, '리뷰를 불러오는 중...')\n"
        "                  : null,\n"
        "                reviewModal.error\n"
        "                  ? h('div', { className: 'meta error' }, reviewModal.error)\n"
        "                  : null,\n"
        "                !reviewModal.loading && !reviewModal.error\n"
        "                  ? h(React.Fragment, null,\n"
        "                      ...((reviewModal.tab === 'positive' ? reviewModal.positive : reviewModal.negative).map((rv, i) => h('div', { className: 'review-item', key: (rv.review_id || i) + '-' + i },\n"
        "                        h('div', null, rv.text_ko || rv.text || ''),\n"
        "                        h('div', { className: 'review-meta' }, '공감 ' + (rv.votes_up || 0) + ' | 플레이 ' + (rv.playtime_forever || 0) + '분 | ' + (rv.review_date || ''))\n"
        "                      ))),\n"
        "                      (reviewModal.tab === 'positive' ? reviewModal.positive : reviewModal.negative).length === 0\n"
        "                        ? h('div', { className: 'meta' }, '리뷰가 없습니다.')\n"
        "                        : null\n"
        "                    )\n"
        "                  : null\n"
        "              )\n"
